...
package threejava.game;

import java.util.LinkedList;
import java.util.Random;
import threejava.frame.GameBoardPanel;
...
public class GameStatus {
    ...
    private ThreeBlock[][] createNewGameBoard(){
        ...
        for(int i = 1 ; i < 5 ; i++){
            for(int j = 1 ; j < 5 ; j++){
                tempblocks[i][j] = new ThreeBlock(0);
    ...
    public void generateNextBlock(){
        ...
        nextBlock = new ThreeBlock(number);
        nextBlock.setX(0);
        nextBlock.setY(0);
    }
    
    private int decideNextBlockColor(ThreeBlock nowBlock,int whiteCount,int blueCount,int redCount,int maxNumber) {
        int nowNumber = -1;
        if(nowBlock != null){
            nowNumber = nowBlock.getNumber();
        }
        int[] weights = {5, 5, 5};    //weights for white, blue and red 
        for(int i=0;i<3;i++){
            if(i!=nowNumber){
...
//        System.out.println(weights[0]+" "+weights[1]+" "+weights[2]);
    ...
    public ThreeBlock[][] getBlocks() {
        return blocks;
    }

    public void setBlocks(ThreeBlock[][] tempBlocks) {
        this.blocks = tempBlocks;
    }
    
    public ThreeBlock getBlock(int i, int j){
        return this.blocks[i][j];
    }

    public ThreeBlock getNextBlock() {
        return nextBlock;
    }

    public void setNextBlock(ThreeBlock nextBlock) {
